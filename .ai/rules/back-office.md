...
## Shared booking row-actions trait + header GlobalSearch
Booking annuler / transférer / modifier / rembourser + the "détails" menu are shared between the bus passengers page (BusBookings) and the header customer search (GlobalSearch) via the trait App\Livewire\BackOffice\Concerns\ManagesBookingActions.

Trait owns: showConfirmationModal/pendingBookingId/pendingActionName + confirmationModalCopy + confirmPendingAction (handles cancel-booking / refund-booking; delegates other actions to hostConfirmationModalCopy()/handleHostConfirmationAction() — BusBookings adds collect-ticket-payment); showTransferModal + transferDepartOptions + transferBookingToBus; showEditModal + editableTrajetStops + saveBookingEdit; flashStatusMessage/flashErrorMessage. Every mutation calls an untouched BookingController method. Host implements resolveBookingForAction(int): Booking (scoping/authorisation — BusBookings: $bus->bookings()->findOrFail; GlobalSearch: $customer->bookings()->findOrFail) and refreshBookingRows(). A not-in-scope id flashes "introuvable" instead of a 404 (resolveBookingForActionOrFlash).

Shared blade: partials/booking-row-actions.blade.php (the 4 action buttons; params canModify/canCancel/canTransfer) and partials/booking-action-modals.blade.php (the 3 modals — bodies gated behind @if($show*Modal) so the computed data is NOT fetched on every idle page render; important because GlobalSearch is embedded in the layout header on every back-office page). partials/customer-bookings-table.blade.php is the search dialog's per-tab table.

GlobalSearch (livewire:back-office.global-search in the layout header before Déconnexion, desktop only): collapsed magnifier -> input; updatedSearchQuery() auto-fires search() when the value matches the Senegalese mobile regex; hit -> flux:modal with header (nom, tel, "client depuis" fr date, total) + tabs Réservations (current) / Voyages passés (history, transfer-only) / Paiements (placeholder); miss -> dispatch('global-search-no-result') -> Alpine red toast, no dialog. Booking rows are mapped in-component (mapBookingRow) — a subset of BookingResource WITHOUT formatted_schedule, which firstOrFail()s a HeureDepart and throws on legacy data. BookingResource itself gained additive busName / departLabel keys.
