...
//                ->where("id","=",35)
    ...
    private function departResource(Depart $depart): array
    {
        $departArray = [
            ...
            'buses' => $this->getBusesForBooking($depart)->map(fn (Bus $bus) => array_merge_recursive([
                'id' => $bus->id,
                ...
                'destinations' => $bus->destinations->count() > 100 ? $bus->destinations->map(function (DestinationBus $item) {
                    return [
    ...
    private ?Collection $availableVehicules = null;

    private function getBusesForBooking(Depart $depart): Collection
    {
        // Bus-selection logic now lives on the model so the public website's CaravaneDepartsResource
        // reuses the exact same rule instead of re-implementing it. The vehicle list is fetched once
        // for the whole response rather than once per départ.
        return $depart->getBusesForBooking($this->availableVehicules ??= Vehicule::all());
    }
