...
>
    <?php if ($placeholder): ?>
        {{-- Overridden from the Flux stub: the placeholder option is deliberately NOT `disabled`.
             When `wire:model` binds a null property, Livewire's updateSelect() deselects every
             option; the browser then falls back to the first *enabled* option, which — with a
             disabled placeholder — is the first real item, so the field looks pre-selected while
             the component value stays null and "required" fires on submit. Keeping the placeholder
             enabled makes that fallback land on the placeholder itself. --}}
